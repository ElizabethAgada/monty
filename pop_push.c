...
void se_push(stack_t **stack, unsigned int line_num)
{
	...
	if (fresh == NULL)
	{
		// Print an error message if malloc fails, free the stack, and exit with failure status
		printf("Error: malloc failed\n");
		free_stack(stack);
		exit(EXIT_FAILURE);
	}

	// Choose wether to push to the stack or queue based on the value of se_stackqueue
